...
- **Stage** — The unit of restart, backup, and constraint application in `workflow.manager`. Each stage writes a `STAGE_{name}_DONE` sentinel on completion; on resubmission completed stages are skipped. A stage owns its constraints and its parameter overrides. Stages are what a submission script composes into an ordered sequence.
- **Step** — A sub-phase *within* a single stage that shares geometry, constraints, and (for ASE-optimizer workflows) one VaspInteractive context. Example: a warm-start single point followed by an optimization that reads its WAVECAR. Not an independent restart unit.
- **Stage template** — A reusable, parameterized stage body defined in a new top-level `stage_templates:` YAML section (parallel to `workflows:`). A submission script instantiates a template into a concrete stage by giving it a required explicit `name=` (which becomes its sentinel/backup/restart key) plus optional per-instance parameter overrides. One template, instantiated N times with different overrides, replaces N near-duplicate `workflows:` entries. (Supersedes the ad-hoc "opt_step" wording.)
- **Production stage** — The canonical final optimization, instantiated from the template designated `'production'` (default name, configurable via the `production=` arg of `run_stages`). Its parameters come straight from YAML with no per-instance overrides, so they stay authoritative. The engine emits a non-blocking warning if the production stage is not the last stage actually run. Distinct from **pre-optimization stages**, the looser scaffolding stages (lower ENCUT, scaffolding constraints) a submission script composes ahead of it.
- **Engine** — What actually performs a stage's optimization. `vasp` (default; the existing VASP/VaspInteractive path) or `mlip`. Declared per stage template via `engine:`.
- **MLIP stage** — A stage with `engine: mlip` that relaxes the structure with a machine-learning interatomic potential via the `mlip_platform` package instead of VASP. Because MLIP packages pin mutually incompatible torch stacks (see mlip_platform ADR 0001), it cannot run in the VASP job env; it executes as a **subprocess** in a separate **MLIP env**. The manager hands it the current structure file plus the stage's constraint config; a thin asetools-provided runner inside the MLIP env re-applies the same JSON constraints (`ConstraintManager`), attaches the MLIP calculator (`setup_calculator`), and runs `run_optimization`. Useful as a fast pre-optimization ahead of VASP stages. **Invariant: MLIP stages are pre-optimization only — every MLIP stage must precede every VASP stage.** `run_stages` enforces this with a hard error at validation time (an MLIP stage after a VASP stage would silently load the wrong structure, since MLIP produces no OUTCAR and `load_structure` is OUTCAR-first). Given the invariant, the MLIP runner just writes a `CONTCAR`; the next VASP stage's `load_structure` finds no OUTCAR yet and reads that CONTCAR. `load_structure` is left unchanged.
- **MLIP env registry** — A `globals.mlip_envs:` map from a short env key to a concrete Python interpreter path. An MLIP stage template references an env by key (`env: uma`); only the `globals` block changes when moving between machines (laptop / ASPIRE2A / cos-cluster). Resolves to an explicit interpreter path, avoiding `conda activate` subshell fragility.
