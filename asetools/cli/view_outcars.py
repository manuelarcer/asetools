...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize the last configuration in OUTCAR files."
    )
    parser.add_argument(
        "--converged", action="store_true", help="Only consider converged calculations"
    )
    parser.add_argument(
        "--folders",
        "-f",
        nargs="+",
        help="List specific folders to process. If not provided, all folders in the current directory will be processed.",
    )
    parser.add_argument(
        "--pyatoms", action="store_true", help="Assume that Pyatoms was not used for this run"
    )
    ...
    for f in valid_folders:
        if args.pyatoms:  # Pyatoms was used
            ...
            ste_dirs = [
                ...
                if d.startswith("step_0") and os.path.isdir(os.path.join(f, d))
            ]
            if not ste_dirs:
