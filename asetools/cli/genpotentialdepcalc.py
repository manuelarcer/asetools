...
def main():
    parser = argparse.ArgumentParser(
        description="Preparation of folders for Potential-dependent calculations"
    )
    parser.add_argument("poscar", help="Path to POSCAR file")
    parser.add_argument("pythonfile", help="Script used for the calculations")
    parser.add_argument(
        "--lower", type=float, default=-1, help="Lower limit for the number of electrons"
    )
    parser.add_argument(
        "--higher", type=float, default=1, help="Higher limit for the number of electrons"
    )
