...
Spin treatment options (for --band-center):
  ...
  down       Only spin-down
    ...
    # Input/output
    parser.add_argument("--doscar", default="DOSCAR", help="Path to DOSCAR file (default: DOSCAR)")
    parser.add_argument(
        "-o",
        "--output",
    ...
    # What to plot
    parser.add_argument(
        "--atoms", help='Atom indices: "0", "0,2,5", "0-5", or "all" (default: plot total DOS)'
    )
    parser.add_argument(
        "--orbitals", help="Orbitals: s, p, d, t2g, eg, all (comma-separated for multiple)"
    )
    parser.add_argument(
        "--total", action="store_true", help="Plot total DOS (overrides --atoms/--orbitals)"
    )
    parser.add_argument(
        "--overlay",
        action="store_true",
    ...
    # Appearance
    parser.add_argument(
        "--xlim", help='Energy range in eV, e.g., "-5,5" (default: full data range)'
    )
    parser.add_argument("--ylim", help='DOS range, e.g., "-10,10" (default: auto)')
    parser.add_argument("--title", help="Plot title (default: auto-generated)")
    parser.add_argument(
        "--dpi", type=int, default=300, help="Resolution for saved figure (default: 300)"
    )
    parser.add_argument(
        "--figsize", default="8,6", help='Figure size in inches as "width,height" (default: "8,6")'
    )
    parser.add_argument("--linewidth", type=float, default=1.5, help="Line width (default: 1.5)")
    parser.add_argument("--colors", help='Custom colors (comma-separated), e.g., "red,blue,green"')
    parser.add_argument(
        "--same-color-spins", action="store_true", help="Use same color for spin-up and spin-down"
    )
    ...
    parser.add_argument(
        "--energy-range", help='Energy range for band center calculation, e.g., "-10,0"'
    )
    parser.add_argument(
        "--spin-treatment",
            ...
            if args.overlay and len(orbitals) > 1:
                ...
                for i, orbital in enumerate(orbitals):
                    ...
                    if pdos_up.sum() == 0 and pdos_down.sum() == 0:
                        ...
                        print(
                            "    This may indicate an invalid orbital name or atoms with no electrons in these orbitals"
                        )
                    ...
                    if args.same_color_spins:
                        ...
                        ax.plot(
                            energy, pdos_down, color=color, linewidth=args.linewidth, linestyle="-"
                        )
                    else:
                ...
                if args.title:
                    ...
                    if len(atoms) == dos.natoms:
                        ...
                        atom_str = f"atoms {','.join(map(str, atoms[:3]))}" + (
                            "..." if len(atoms) > 3 else ""
                        )
