...
def extract_corrected_energy_fermie(listfolders, calc_zero):
    ...
    for folder in listfolders:
        if check_outcar_convergence(os.path.join(folder, "OUTCAR"), verbose=False):
            ...
            energy_corr = energy_original - (nelect - ref_nelect) * (
                fermi_shift if fermi_shift else 0
            )
...
def get_energy_at_givenpotential(results, e_ref=None, order=2, desiredU=0.0):
