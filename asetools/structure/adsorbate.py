...
class SurfaceAnalyzer:
            ...
            for j in range(i + 1, len(surface_indices)):
                ...
                if d_ij < thr:
                    for k in range(j + 1, len(surface_indices)):
                        d_ik = self.atoms.get_distance(
                            surface_indices[i], surface_indices[k], mic=True
                        )
                        d_jk = self.atoms.get_distance(
                            surface_indices[j], surface_indices[k], mic=True
                        )
                        if d_ik < thr and d_jk < thr:
                            surface_neighbors.append(
                                (surface_indices[i], surface_indices[j], surface_indices[k])
                            )
    ...
    def get_cluster_around_adsorbate(self, thr: float = 4.4) -> Optional[Atoms]:
        try:
            dist_mtx = self.atoms.get_distances(
                self.adsorbateindex, indices=range(len(self.atoms)), mic=True
            )
    ...
    def adsorbate_to_cluster_neighbors(
        ...
        for s in self.symbols:
            dist_neigh = self.adsorbatecluster.get_distances(
                len(self.adsorbatecluster) - 1, range(len(self.adsorbatecluster)), mic=True
            )
            dist_neigh = dist_neigh[
                (dist_neigh > 0.01) & (np.array(self.adsorbatecluster.get_chemical_symbols()) == s)
            ]
